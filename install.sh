  ...
  for pair in "progress.md:.agents/graph/sessions/progress.md" \
    ...
    if [ -f "$old_path" ]; then
      if [ -e "$new_path" ]; then
        ...
        {
          echo "<!-- Migrado autom?ticamente desde $old_path el $today -->"
          echo
          cat "$old_path"
        } > "$new_path"
        echo "  [ok]   copiado: $old_path -> $new_path (se preserv? el original)"
      fi
    fi
  done
fi
...
safe_copy "$SCRIPT_DIR/scripts/check-text-encoding.py" "scripts/check-text-encoding.py"
