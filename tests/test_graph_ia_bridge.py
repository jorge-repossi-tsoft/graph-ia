...
SCRIPT_PATH = Path(__file__).resolve().parents[1] / 'scripts' / 'graph-ia.py'
SPEC = importlib.util.spec_from_file_location('graph_ia', SCRIPT_PATH)
graph_ia = importlib.util.module_from_spec(SPEC)
assert SPEC.loader is not None
sys.modules[SPEC.name] = graph_ia
SPEC.loader.exec_module(graph_ia)
...
class BridgeSyncTests(unittest.TestCase):
    ...
    def test_fresh_install_writes_single_bridge_block(self):
        ...
        graph_ia.place_bridge(str(self.repo), "AGENTS.md", report, legacy_exists=False)

        content = (self.repo / "AGENTS.md").read_text(encoding="utf-8")
        self.assertEqual(content.count(graph_ia.BRIDGE_MARKER), 1)
        self.assertEqual(content.count(graph_ia.BRIDGE_END_MARKER), 1)
    ...
    def test_update_resyncs_stale_block_and_preserves_user_content(self):
        ...
        agents_path.write_text(
            ...
            f"{graph_ia.BRIDGE_MARKER}\n"
            "Contenido viejo/desactualizado del bridge.\n"
            f"{graph_ia.BRIDGE_END_MARKER}\n"
            "\n## Notas del equipo\nEsto no debe borrarse.\n",
            encoding="utf-8",
        )

        report = _fresh_report()
        graph_ia.place_bridge(str(self.repo), "AGENTS.md", report, legacy_exists=False, update=False)
        ...
        report = _fresh_report()
        graph_ia.place_bridge(str(self.repo), "AGENTS.md", report, legacy_exists=False, update=True)
    ...
    def test_update_is_idempotent(self):
        ...
        graph_ia.place_bridge(str(self.repo), "AGENTS.md", report2, legacy_exists=False, update=True)
    ...
    def test_legacy_line_included_only_when_legacy_system_exists(self):
        report = _fresh_report()
        graph_ia.place_bridge(str(self.repo), "AGENTS.md", report, legacy_exists=True)
        content = (self.repo / "AGENTS.md").read_text(encoding="utf-8")
